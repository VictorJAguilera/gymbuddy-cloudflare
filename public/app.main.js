...
if (window.__GB_APP_ALREADY_LOADED__) {
  try{ window.__gb_overlay && __gb_overlay('Core: ya cargado, omito'); }catch(e){}
  ...
  // Refs DOM globales (UNA sola declaración; asignación en boot)
  ...
  function EditExerciseCard(r, x){
    ...
    // En edición: mostrar valores guardados (si null, caja vacía)
  ...
  // Navegación con animación pedida
   ...
   * ------------------------------------- */
  function boot() {
    try{ window.__gb_overlay && __gb_overlay('Core: boot'); }catch(e){}
    appEl = document.getElementById('app');
    if (!appEl) { try{ window.__gb_overlay && __gb_overlay('Falta #app en DOM'); }catch(e){}; return; }
    ...
    try{ window.__gb_overlay && __gb_overlay('Core: render(' + (STATE && STATE.view) + ')'); }catch(e){}
  }
